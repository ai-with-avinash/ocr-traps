...
\title{An OCR Evaluation Framework for Enterprise Documents: Evidence from a Locked 90-Document Phase~1 Benchmark}
...
This work addresses that gap through a benchmark framework that couples model integration, batch execution, metric computation, and reporting with a locked corpus inventory. Earlier project materials referenced a larger intended corpus, but the checked-in repository currently yields 90 reproducible visible OCR inputs under a shared inventory rule. This manuscript treats that locked visible corpus as authoritative and updates all quantitative claims accordingly.
...
\begin{enumerate}
    ...
    \item It locks the current repository corpus at 90 visible OCR inputs and emits a canonical corpus summary artifact that ties corpus counts, ground-truth coverage, and run coverage to submission claims.
...
\begin{table*}[t]
\centering
\caption{Phase~1 model set and run coverage on the 90-document visible corpus. Mean latency is computed over successful visible-corpus runs. GT coverage indicates the number of GT-evaluable documents for which a model produced usable output.}
\label{tab:coverage}
\footnotesize
\resizebox{\textwidth}{!}{%
...
Operational coverage is reported as successful runs over the full 90-document visible corpus. Accuracy metrics are reported only on GT-evaluable documents. Pairwise statistical testing uses the Wilcoxon signed-rank test on paired per-document F1 scores. Forms-only comparisons are reported separately because the unbiased subset is small ($n=5$). Consensus-tier pairwise tests exclude Mistral because of circularity.
...
\begin{table}[t]
\centering
\caption{Human-verified forms results ($n=5$). This is the only unbiased cross-model accuracy table in the paper.}
\label{tab:forms}
\footnotesize
